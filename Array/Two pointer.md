...
**Main goal:** reduce unnecessary / repeated work.
...
## 4) Sorted Array + Two Pointers (classic)
...
**Important:** the movement rule comes from sorting. Without sorted order, this logic is usually not valid.
...
Both generally move from left to right, but they can have **different jobs**.
...
**Important:** the region `[slow...fast]` is **not necessarily a window**.
So: **Read/Write Two Pointers ≠ Sliding Window**
...
**Important:** here we are keeping **all `n` elements**, so the zeroes must be filled at the end.
...
Here `[left...right]` itself is important (a contiguous range).
...
Example: shortest subarray with sum ≥ target (**when all elements are positive**)
...
the current window is valid. Since we want the **shortest** one:
...
So: **do not blindly use a basic sliding window for arbitrary integers.**

Example: Longest Subarray With Sum K → common solution:
...
**Two Pointers:** general technique (two indexes moved intelligently).

**Sliding Window:** a specific two-pointer pattern where `[left...right]` is the current contiguous window.
...
4. **Monotonic behavior** (e.g., positive numbers)
...
**Golden question:** *“What am I eliminating by moving this pointer?”*
...
Two Pointer variables usually require:
...
| Sliding Window | `right` expands; `left` shrinks when needed |
...
## 22) One-line definitions (quick revision)

- **Two Pointers:** use two indexes and move them intelligently to eliminate unnecessary work.
- **Opposite pointers:** start from both ends and move toward each other.
- **Same-direction pointers:** both move left to right, often with different roles.
- **Read/Write:** `fast` reads; `slow` writes / maintains the valid portion.
- **Sliding Window:** maintain a meaningful contiguous range `[left...right]`.
- **Sorting:** often provides the ordering needed to make pointer movement safe.
- **Pointer movement:** move only when you can explain why it is safe.
- **Time:** usually `O(n)` when each pointer moves at most `O(n)` times.
- **Space:** usually `O(1)` auxiliary space unless extra structures are used.
